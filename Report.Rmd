...
We consider three models: exponential, Weibull, and Cox. The exponential and Weibull accelerated failure time (AFT) models implement different baseline hazard functions, while the Cox proportional hazard model estimates $\beta$ without this specification. Notably, even if the baseline hazard function is known, the Cox model is still expected to perform well due to its semi-parametric efficiency (Anderson and Gill). It is important to note that the Exponential model is nested within the Weibull model, as an exponential distribution is a special case of the Weibull distribution where $\gamma = 1$. All models impose the restraint that the effect of the treatment be multiplicative on the hazard curve. The exponential and Weibull hazard curves can be seen below, in Table 1.\par
...
# References 
https://www-jstor-org.ezproxy.cul.columbia.edu/stable/2240714?pq-origsite=summon&seq=1#metadata_info_tab_contents _ need to add citation
