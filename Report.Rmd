...
\small Figure 1. Baseline hazard functions used in this study for data generation. The lognormal distribution uses $\mu=0$ and $\sigma=0.5$. The piecewise distribution is based on the curve generated by Brilleman et al. The exponential and Weibull($\gamma=0.5$) curves were fit using $\lambda=0.1$. The Weibull distributions with $\gamma = 2,5$ have been omitted due to scaling (very steep growth).
