...
MSE_table = tibble(
  ...
  Gamma_2 = c(MSE(result_1$exp_beta), MSE(result_1$weibull_beta), MSE(result_1$cox_beta)),
...
```
To assess model performance, we used mean-squared error (MSE) and ____. We also plotted the survival probability verse time of each model, with reference lines.
...
The following table presents mean squared error of beta of each model.
```{r MSE_beta_table, echo=FALSE}
MSE_table
```
