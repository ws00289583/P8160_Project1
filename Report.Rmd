...
# Results
To assess model performance, we plotted the survival probability versus time of all models with each simulation data. We also used mean-squared error (MSE) to check model performance.

```{r suv vs time (0.5), echo=FALSE}
...
```{r suv vs time (1), echo=FALSE}
pp <- readPNG("Gamma_1.png")
plot.new() 
rasterImage(pp,0,0,1,1)
```

Figure 3. Survival probability versus time of each model, baseline hazard function follows weibull distribution with $\gamma = 1$.

```{r suv vs time (2), echo=FALSE}
...
By comparing the Mean square error(MSE) and mean of $\beta$ derived from three models in diffrent simulations, we found that misspecifying the baseline hazard function can lead to significant differences between actual value of $\beta$ and the truth(which was set as -0.5). 
