...
Table 1. Baseline hazard functions, $h_0(t)$, for the continuous models considered in this study. The normal CDF is denoted by $\Phi$.

# Simulation Design

We simulated survival data

All simulation data was generated using the `simsurv` function in the `simsurv` package. We defined a binomial treatment variable (`trt`) with $p=0.5$ to ensure equal likelihood of random assignment to the treatment or control group. For the Weibull model, we considered various parameters ($\gamma=0.5,1,2,5$, where $\lambda=1$ denotes the exponential model. We used $\beta = -0.5$ as the true effect size.
